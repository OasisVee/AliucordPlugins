...
public class InvisibleMessages extends Plugin {
            ...
            try {
                ...
                tw.setOnClickListener(v1 -> {
                    ...
                    dialog.setOnOkListener(v2 -> {
                        ...
                        if (in.isEmpty() || in.equals("Password")) removePassword(channelId);
                        else addPassword(channelId, in);
                        actions.dismiss();
                    });
                });
                layout.addView(tw);
    ...
    private void registerCommand() {
        var options = Arrays.asList(
                Utils.createCommandOption(ApplicationCommandType.STRING, "message", "This is what normal people see", null, true),
                Utils.createCommandOption(ApplicationCommandType.STRING, "hiddenMessage", "Hidden message,only people that has the password can see this", null, true),
        ...
        commands.registerCommand("invis", "Send A Invisible Message", options, ctx -> {
            ...
            try {
                ...
                logger.error(e);
                return new CommandsAPI.CommandResult("An Error Occurred, Check Debug log for more info", null, false);
            }
        });
    }

    private void patchItemMessage() throws NoSuchMethodException {
        patcher.patch(WidgetChatListAdapterItemMessage.class.getDeclaredMethod("configureItemTag", Message.class, boolean.class), new Hook((cf) -> {
                ...
                tw.setOnClickListener((v) -> {
                    ...
                    dialog.setOnOkListener(v1 -> {
                        new Thread(() -> {
                            ...
                            try {
                                decryptedMessage = InvChatAPI.decrypt(message.getContent(), input);
                            } catch (Exception e) {
                                logger.error(e);
                                decryptedMessage = "Message couldn't be decrypted, You can check Debug Log for more info";
                            }

                            var embed = new MessageEmbedBuilder().setTitle("Decrypted Message").setDescription(decryptedMessage).build();
                        ...
                        if (input.isEmpty()) {
                            ...
                            new Thread(() -> {
                                try {
                                    ...
                                    Utils.mainThread.post(() -> Toast.makeText(context, "An Error Occurred, Message Couldn't be sent", Toast.LENGTH_SHORT).show());
                                    logger.error(e);
                                }
                            }).start();
                            dialog.dismiss();
